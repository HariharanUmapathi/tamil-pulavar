...
if($process == 'delete')
{
	mysqli_query($connection,"DELETE FROM `proverbs` WHERE `sno`='$id'")or die(mysqli_error($connection));
...
{
	mysqli_query($connection,"UPDATE `proverbs` SET `admin_checked`='1' WHERE `sno`='$id'")or die(mysqli_error($connection));
