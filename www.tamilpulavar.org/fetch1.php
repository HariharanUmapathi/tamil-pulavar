...
<body>
	...
	$qry_lst = mysqli_query($connection, "SELECT `table_name` FROM `table_name` WHERE `language`='tam'");
	...
	while ($fet_val = mysqli_fetch_assoc($qry_lst)) {
		...
		if (!in_array($table, $unused_tab)) {
			$qry	    =	mysqli_query($connection, "SELECT `meaning` FROM `$table` WHERE `tword` = '$val'") or die(mysqli_error($connection));
