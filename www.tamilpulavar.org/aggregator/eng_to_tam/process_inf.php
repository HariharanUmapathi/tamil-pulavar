...
$inf_en			   =	T2R($inf);
$s_en				 =	T2R($s);
$inf0				 =	str_replace("_kka","",$inf_en);
if($inf0 == $inf_en)
{
$inf1 				 =	substr($inf_en,0,-2);
if(substr($inf1,-2) == "_y" || substr($inf1,-2) == "_r" || substr($inf1,-2) == "_Z" || substr($inf1,-2) == "_V" || substr($inf1,-2) == "_L" 
|| substr($inf1,-2) == "_l")
	{
		$inf0		 =	$inf1;
	}
	...
	{
		if(substr($inf_en,-3) == "_TTa")
		$inf0		 =	str_replace("_TTa","Tu",$inf_en);
		else
		$inf0		 =	$inf_en."u";
	}
}
...
$fet_enwd	 =	mysqli_fetch_array($qen_wrd);
if(count($f_ev1) < count($inf_f))
{
	foreach($inf_f as $inf_fe)
	{
		foreach($f_ev1 as $f_e)
		{
			//echo $f_e;
			
		echo $fet_enwd['eword']." ".$f_e." "."for ".$inf_fe;
		echo "</br>";
