...
$table = "sentance";
ExportExcel($table, $connection);

function ExportExcel($table, $connection)
{
	$filename = "uploads/" . strtotime("now") . '.xls';
	header("Content-type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=$filename");

	$sql = mysqli_query($connection, "SELECT `sentances` FROM $table") or die(mysqli_error($connection));
