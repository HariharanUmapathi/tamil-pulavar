...
<body>
	...
	$prev	 = 	$ft_sq['api_hits'];
	mysqli_query($connection, "UPDATE `api_hits` SET `api_hits` = $prev+1 WHERE `sno` = 1") or die(mysqli_error($connection));
	//$cate	   =	$_GET['cate'];
					...
					<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
						...
						for ($i = 0; $i < 10; $i++) {
							...
							<div class="panel panel-default">
								<div class="panel-heading" role="tab" id="<?php echo "head_" . $i; ?>">
									<h4 class="panel-title panel-danger">
										<a role="button" data-toggle="collapse" data-parent="#accordion" href="#<?php echo $i; ?>"
											aria-expanded="<?php if ($i == 0) {
																echo "true";
															} else {
																echo "false";
															} ?>"
											aria-controls="<?php echo $i; ?>" class="text-danger <?php if ($i == 0) {
																									} else {
																										echo "collapsed";
																									} ?>">
											<?php echo $nam; ?></a>
								...
								<div id="<?php echo $i; ?>" class="panel-collapse <?php if ($i == 0) {
																						echo "collapse in";
																					} else {
																						echo "collapse";
																					} ?>" role="tabpanel" aria-labelledby="<?php echo "head_" . $i; ?>">
					...
					<div class="panel-group" id="accordion1" role="tablist" aria-multiselectable="true">
						...
						for ($s = 11; $s < 20; $s++) {
							...
							<div class="panel panel-default">
								<div class="panel-heading" role="tab" id="<?php echo "head1_" . $s; ?>">
									<h4 class="panel-title panel-danger">
										<a role="button" data-toggle="collapse" data-parent="#accordion1" href="#<?php echo $s; ?>"
											aria-expanded="<?php if ($s == 11) {
																echo "true";
															} else {
																echo "false";
															} ?>"
											aria-controls="<?php echo $s; ?>" class="text-danger <?php if ($s != 11) {
																										echo "collapsed";
																									} ?>">
											<?php echo $nam1; ?>
										</a>
								...
								<div id="<?php echo $s; ?>" class="panel-collapse <?php if ($s == 11) {
																						echo "collapse in";
																					} else {
																						echo "collapse";
																					} ?>" role="tabpanel" aria-labelledby="<?php echo "head1_" . $s; ?>">
	...
	<div class='row'><div class='col-md-10 col-md-offset-1'><table class='table table-responsive table-striped table-bordered '><tr><td>";
		/*$sql	   =	mysqli_query("SELECT `sentence` FROM `sentence` WHERE `sentence` LIKE '%$val%' LIMIT 4");
...
exit;*/
			...
			/*if($h%2 == 0)
	{
		...
		$qry_lst = mysqli_query($connection, "SELECT `table_name` FROM `table_name` WHERE `language`='eng'");
		$table_len  =    mysqli_num_rows($qry_lst);
		$i = 0;

		while ($fet_val = mysqli_fetch_assoc($qry_lst)) {
			$table	  =	$fet_val['table_name'];
			$qry	    =	mysqli_query($connection, "SELECT `meaning` FROM `$table` WHERE `eword` = '$val'") or die(mysqli_error($connection));
			$qry1	   =	mysqli_query($connection, "SELECT `full_name`,`link` FROM `table_name` WHERE `table_name` = '$table'") or die(mysqli_error($connection));
			$fet_tabl   =	mysqli_fetch_array($qry1);
			$table_name =	$fet_tabl['full_name'];
			$link 	   = 	$fet_tabl['link'];
